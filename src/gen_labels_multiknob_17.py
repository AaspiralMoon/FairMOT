...
def xyxy2xywh(x):
    # Convert bounding box format from [x1, y1, x2, y2] (bottom left, top right) to [x, y, w, h] (top, left)
    y = np.zeros(x.shape)
    y[:, 0] = x[:, 0]
    y[:, 1] = x[:, 1]
    y[:, 2] = x[:, 2] - x[:, 0]
...
for seq in seqs:
    ...
    for qp in qp_list:
        seq_label_root = osp.join(label_root, seq, 'qp_{}'.format(qp))
        output_path = osp.join(seq_root, seq, 'qp_{}'.format(qp))
        mkdir_if_missing(output_path)
        for fid, f in enumerate(os.listdir(seq_label_root)):
            print('working on {} knob: {} image: {}'.format(seq, qp, int(fid + 1)))
