...
def gen_detections(data_root, result_root, model_root, seqs, qp_list):       # for qp
    for seq in seqs:
        for qp in qp_list:
            img_path = osp.join(data_root, seq, 'QP_{}'.format(qp), 'images')
            result_path = osp.join(result_root, seq, 'qp_{}'.format(qp))
            mkdir_if_missing(result_path)
            cmd_str = 'python gen_detections.py --data_dir {} --output_root {} --load_model {}/model_last.pth --gen_dets'.format(img_path, result_path, model_root)
            os.system(cmd_str)
...
if __name__ == '__main__':
    seqs = ['MOT17-02-SDP',
            ...
            'MOT17-11-SDP',
            'MOT17-13-SDP']
    model_root = '../exp/mot/mot17_half_full-dla34_coco_crowdhuman'
    ...
    gen_detections(data_root, result_root, model_root, seqs, qp_list)